...
class Visuals(ShowBase):
    ...
    def __init__(self):
        ...
        for (i, j) in zip(self.results['delta'][1:], self.results['phi'][1:]):
            ...
            bf_hpr = self.back_frame.hprInterval(0.01, (self.v * sin(pi * i / 180) / 5, 0, j-prev), other=self.back_frame)
    ...
    def camera_task(self, task):
        self.camera.setPos(self.back_frame.getPos() + (-30 * sin(pi * self.back_frame.getH() / 180), 30 * cos(pi * self.back_frame.getH() / 180), 10))
